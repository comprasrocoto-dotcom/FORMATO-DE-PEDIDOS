...
export async function actualizarNumeroPedidoSistema(data: NumeroPedidoSistemaRow): Promise<{ ok: boolean; error?: string }> {
      const payload = { action:'actualizarNumeroPedidoSistema', nOrden:data.nOrden||'', numeroPedidoSistema:data.numeroPedidoSistema||'' };
      try {
              const res = await fetch(APPS_SCRIPT_URL, { method:'POST', headers:{'Content-Type':'text/plain'}, body:JSON.stringify(payload), redirect:'follow' });
              const text = await res.text().catch(()=>'');
              if(!res.ok) return { ok:false, error:'HTTP '+res.status };
